...
export class TradeController {
	...
	@ApiParam({ name: 'coin', description: '코인 이름', example: 'BTC' })
	@ApiResponse({ status: HttpStatus.OK, description: '사용자 보유 코인' })
	@ApiResponse({
		status: HttpStatus.CREATED,
		description: '사용자 미보유 코인',
	})
	async getMyCoinData(@Request() req, @Param('coin') coin: string) {
	...
	async getMyTradeData(@Request() req, @Param('coin') coin?: string) {
