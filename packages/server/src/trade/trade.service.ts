import { Injectable, OnModuleInit, UnauthorizedException,UnprocessableEntityException } from '@nestjs/common';
...
export class TradeService implements OnModuleInit{
    ...
    async buyTradeService(buyDto, matchPending=false){
        ...
        try {
            ...
            await this.assetRepository.createAsset(buyDto, currentCoinPrice, account, queryRunner)

            const accountId = account.accountId
            await this.accountRepository.succesBuy(buyDto,afterTradeBalance,accountId,queryRunner)
    ...
    async checkCurrency(buyDto){
        ...
        if(!accountResult) throw new UnprocessableEntityException({
            response: {
              message: "자산이 부족합니다.",
              statusCode: 422,
            },
    ...
    async matchPendingTrades() {
        try {
            ...
            if (this.matchPendingTradesTimeoutId) clearTimeout(this.matchPendingTradesTimeoutId);
            this.matchPendingTradesTimeoutId = setTimeout(() => this.matchPendingTrades(), UPBIT_UPDATED_COIN_INFO_TIME);
        }
    }
}
