...
export class ChartService implements OnModuleInit {
	...
	async waitForTransactionOrder(key, maxRetries = 100) {
		// 10초 타임아웃
			...
			if (key === 'minutes') {
				...
				keys.forEach(async (min) => {
					...
					if (!candleData) {
						...
						candleData.high_price =
							candleData.high_price < price ? price : candleData.high_price;
						candleData.low_price =
							candleData.low_price > price ? price : candleData.low_price;
				...
				const candleData =
					await this.chartRepository.getSimpleChartData(redisKey);
				if (!candleData) {
					...
					this.chartRepository.setChartData(
						redisKey,
