...
export class FavoriteController {
  ...
  @ApiOperation({ summary: '즐겨찾기 조회', description: '사용자의 즐겨찾기 목록을 조회합니다.' })
  @ApiQuery({ 
    name: 'assetName', 
    required: false, 
    type: String,
  ...
  async getFavorites(
    ...
    this.logger.log(`즐겨찾기 조회 시작: userId=${req.user.userId}, assetName=${assetName || 'all'}`);
    try {
  ...
  @ApiOperation({ summary: '즐겨찾기 추가', description: '새로운 즐겨찾기를 추가합니다.' })
  @ApiQuery({ 
    name: 'assetName', 
    required: true, 
    type: String,
  ...
  async createFavorite(
    ...
    this.logger.log(`즐겨찾기 추가 시작: userId=${req.user.userId}, assetName=${assetName}`);
    try {
  ...
  @ApiOperation({ summary: '즐겨찾기 삭제', description: '즐겨찾기를 삭제합니다.' })
  @ApiQuery({ 
    name: 'assetName', 
    required: true, 
    type: String,
  ...
  async deleteFavorite(
    ...
    this.logger.log(`즐겨찾기 삭제 시작: userId=${req.user.userId}, assetName=${assetName}`);
    try {
  ...
  async toggleFavorite(
    ...
    this.logger.log(`즐겨찾기 토글 시작: userId=${req.user.userId}, assetName=${assetName}`);
    try {
